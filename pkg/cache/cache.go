...
func AddSortedSet(key string, member string, score float64) {
	redisClient := GetRedisClient().RedisClient.Client
	redisClient.ZAdd(redisClient.Context(), key, &redis.Z{Score: score, Member: member})
}

func CountSortedSet(key string) uint64 {
	redisClient := GetRedisClient().RedisClient.Client
	r, _ := redisClient.ZCard(redisClient.Context(), key).Uint64()
	return r
}

func RemRangeByRankSortedSet(key string, start int64, stop int64) {
	redisClient := GetRedisClient().RedisClient.Client
	redisClient.ZRemRangeByRank(redisClient.Context(), key, start, stop)
}

func RangeSortedSet(key string, start int64, stop int64) ([]redis.Z, error) {
	redisClient := GetRedisClient().RedisClient.Client
	return redisClient.ZRangeArgsWithScores(redisClient.Context(), redis.ZRangeArgs{Key: key, Start: start, Stop: stop}).Result()
}

func RevRangeSortedSet(key string, start int64, stop int64) ([]redis.Z, error) {
	redisClient := GetRedisClient().RedisClient.Client
	return redisClient.ZRevRangeWithScores(redisClient.Context(), key, start, stop).Result()
}

func RemSortedSetMember(key string, member string) {
	redisClient := GetRedisClient().RedisClient.Client
	redisClient.ZRem(redisClient.Context(), key, member)
}
